...
from universal_mcp_elevenlabs.app import ElevenlabsApp

@pytest.fixture
def app_instance():
    """Provides a ElevenlabsApp instance for tests."""
    mock_integration = MagicMock()
    mock_integration.get_credentials.return_value = {"access_token": "dummy_access_token"}

    return ElevenlabsApp(integration=mock_integration)

def test_universal_mcp_elevenlabs_app_initialization(app_instance):
    """
    Test that the ElevenlabsApp instance is initialized correctly with a name.
    """
    ...
    assert app_instance.name == "elevenlabs", "ElevenlabsApp instance has unexpected name."


def test_universal_mcp_elevenlabs_tool_docstrings_format(app_instance):
    """
    Test that each tool method in ElevenlabsApp has a well-formatted docstring,
...
def test_universal_mcp_elevenlabs_tools_are_callable(app_instance):
    """
    Test that each tool method returned by list_tools in ElevenlabsApp is callable.
    """
    tools = app_instance.list_tools()
    assert isinstance(tools, list), "list_tools() should return a list."
